...
if purpose=='train':
    n_closed = int(len(mesh_files) * 1.3 * 0.2)
else:
    n_closed = 0
...
for mesh_file in mesh_files:
    ...
    mesh0 = o3d.io.read_triangle_mesh(f"dat/{purpose}/meshes/{mesh_file}")
    ...
    for orientation in orientations:
        ...
        while flag:
            ...
            k1,k2 = int(sys.argv[2])//2, (int(sys.argv[2])//2)+1
            ...
            if len(pcd) >= n_points/5:
                if len(pcd) == len(points):
                    if n_closed > 0:
                        ...
                        print(f"{n_closed} watertight mesh")
                        n_closed -= 1
                        count += 1
                        flag = False
                else:
